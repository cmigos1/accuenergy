...
\begin{figure}[ht]
  ...
  \caption{Fluxo de dados da arquitetura \textit{Edge-to-Server}. O STM32H743
  (ADC e DSP) transmite \textit{frames} bin\'arios ao ESP32 via UART a
  460\,800~baud; o ESP32 publica as medi\c{c}\~oes em formato JSON no
  \textit{broker} Mosquitto via MQTT; o script \texttt{mqtt\_ingest.py}
  persiste os dados em arquivos CSV (medi\c{c}\~oes e harm\^onicas); a
  an\'alise de desagrega\c{c}\~ao (K-means e algoritmo de Hart) \'e executada
  em Python.}
  \label{fig:pipeline}
\end{figure}
\FloatBarrier
...
conjunto com o Wi-Fi \citep{ref8}. Ao reestruturar o projeto de forma h\'ibrida, as
